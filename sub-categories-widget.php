<?php
/*
 * Plugin Name: 	Sub Categories Widget (Oli's fork)
 * Description:		This Widget lists the sub-categories for a given category.
 * Plugin URI:		https://github.com/magicoli/sub-categories-widget
 * Author:			Magiiic
...
class sub_categories_widget extends WP_Widget {
	...
	function __construct() {
		parent::__construct(
			'sub_categories_widget',
			__( 'Sub-categories', 'sub-categories-widget' ),
			array(
				'classname'   => 'widget_sub_categories',
				'description' => __( 'Lists the sub-categories for a given category.', 'sub-categories-widget' ),
			)
		);
	}
	...
	function widget( $args, $instance ) {
		...
		if ( ( $post_is_parent == 0 ) || ( ( $post_is_parent == 1 ) && ( ! empty( $subs ) ) ) ) {
			...
			if ( $dropdown == 0 ) {
				...
				$subs = wp_dropdown_categories(
					array(
						'id'               => 'sub-cat-' . $this->number,
	...
	function form( $instance ) {
		...
		$instance = wp_parse_args(
			(array) $instance,
			array(
				'title'           => __( 'Sub Categories', 'sub-categories-widget' ),
				'category_id'     => 1,
				'use_cat_title'   => 0,
				'hide_empty_cats' => 0,
				'show_post_count' => 1,
				...
				'dropdown'        => 0,
				'post_is_parent'  => 0,
				'dropdown_text'   => __( 'Select Sub-category', 'sub-categories-widget' ),
				'list_order'      => 0,
			)
		);
		...
		?>
			...
			<p>
				...
				<input id="<?php echo $this->get_field_id( 'use_cat_title' ); ?>" name="<?php echo $this->get_field_name( 'use_cat_title' ); ?>" type="checkbox" value="1" 
										<?php
										if ( $instance['use_cat_title'] ) {
											echo 'checked="checked"';}
										?>
				/>
				<label for="<?php echo $this->get_field_id( 'use_cat_title' ); ?>"><?php _e( 'Title is parent category', 'sub-categories-widget' ); ?></label>
				<br>
				<input id="<?php echo $this->get_field_id( 'show_post_count' ); ?>" name="<?php echo $this->get_field_name( 'show_post_count' ); ?>" type="checkbox" value="1" 
										<?php
				...
				<br>
				<input id="<?php echo $this->get_field_id( 'hide_empty_cats' ); ?>" name="<?php echo $this->get_field_name( 'hide_empty_cats' ); ?>" type="checkbox" value="1" 
										<?php
										if ( $instance['hide_empty_cats'] ) {
											echo 'checked="checked"';}
										?>
				/>
				<label for="<?php echo $this->get_field_id( 'hide_empty_cats' ); ?>"><?php _e( 'Hide empty sub-categories', 'sub-categories-widget' ); ?></label>
				<br>
				<input id="<?php echo $this->get_field_id( 'title_link' ); ?>" name="<?php echo $this->get_field_name( 'title_link' ); ?>" type="checkbox" value="1" 
										<?php
				...
				<br>
				<input id="<?php echo $this->get_field_id( 'sub_subs' ); ?>" name="<?php echo $this->get_field_name( 'sub_subs' ); ?>" type="checkbox" value="1" 
										<?php
										if ( $instance['sub_subs'] ) {
											echo 'checked="checked"';}
										?>
				/>
				<label for="<?php echo $this->get_field_id( 'sub_subs' ); ?>"><?php _e( 'Show full sub-category tree', 'sub-categories-widget' ); ?></label>
				<br>
				<input id="<?php echo $this->get_field_id( 'list_order' ); ?>" name="<?php echo $this->get_field_name( 'list_order' ); ?>" type="checkbox" value="1" 
										<?php
										if ( $instance['list_order'] ) {
											echo 'checked="checked"';}
										?>
				/>
				<label for="<?php echo $this->get_field_id( 'list_order' ); ?>"><?php _e( 'Display categories in reverse order', 'sub-categories-widget' ); ?></label>
				<br>
				<input id="<?php echo $this->get_field_id( 'post_is_parent' ); ?>" name="<?php echo $this->get_field_name( 'post_is_parent' ); ?>" type="checkbox" value="1" 
										<?php
										if ( $instance['post_is_parent'] ) {
											echo 'checked="checked"';}
										?>
				/>
			...
			<p>
				<input id="<?php echo $this->get_field_id( 'dropdown' ); ?>" name="<?php echo $this->get_field_name( 'dropdown' ); ?>" type="checkbox" value="1" 
										<?php
...
add_action(
	'widgets_init',
	function () {
		register_widget( 'sub_categories_widget' );
	}
);

load_plugin_textdomain( 'sub-categories-widget', false, basename( __DIR__ ) . '/languages' );
